...
@router.post("/{game_id}/start", response_model=GameResponse, status_code=status.HTTP_200_OK)
...
@router.post("/{game_id}/end", response_model=GameResponse, status_code=status.HTTP_200_OK)
async def end_game(
    game_id: uuid.UUID,
    access_token: str,
    sql: sqlas.AsyncSession = Depends(get_sql_session),
